...
setwd("~/Desktop/Course Documents/ANLY 503/Final")

google = read.csv("google-play-store-apps/googleplaystore.csv")
google_review = read.csv("google-play-store-apps/googleplaystore_user_reviews.csv")
#apple_store = read.csv("apple/AppleStore.csv")
apple_store = read.csv("apple.csv")

#structure
str(apple_store)

asia = apple_store[apple_store$continent == "Asia",]
NorthAmerica = apple_store[apple_store$continent == "North America",]
Europe = apple_store[apple_store$continent == "Europe",]


netdf = apple_store[ apple_store$prime_genre == "Social Networking",]

netdf = subset(netdf,select = c("track_name","user_rating"))

#netdf$to_track_name = sample(asia$track_name,nrow(netdf),replace = TRUE)
#netdf$from_track_name = sample(asia$track_name,nrow(netdf),replace = TRUE)
top_app = c("Facebook","Twitter","LinkedIn","Tinder","Tumblr","Snapchat","Instagram","WhatsApp Messenger",
            "QQ","QZone","Wechat","知乎","百度贴吧","LINE","Weibo","Reddit","Pinterest")


#netdf$to_track_name = sample(netdf$track_name)
netdf$to_track_name = sample(top_app,nrow(netdf),replace = TRUE)

netdf$from_track_name = sample(netdf$track_name,nrow(netdf),replace = TRUE)

edgeList <- subset(netdf,select = c("from_track_name","to_track_name","user_rating"))
...
networkD3::saveNetwork(D3_network_LM, "D3_apple.html", selfcontained = TRUE)
